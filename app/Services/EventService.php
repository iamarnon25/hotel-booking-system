...
class EventService
{
    ...
    // {
    //     // Parse relevant dates and times
    //     $recurringUntil = Carbon::parse($requestData['recurring_until'])->setTime(23, 59, 59);
    //     $start_time = Carbon::parse($requestData['start_time']);
    //     $end_time = Carbon::parse($requestData['end_time']);

    //     // Query the database for events overlapping with the specified time
    //     $events = Event::where('room_id', $requestData['room_id'])
    //         ->where(function ($query) use ($start_time, $end_time) {
    //             $query->where(function ($query1) use ($start_time, $end_time) {
    //                 $query1->where('start_time', '<', $end_time)->where('end_time', '>', $start_time);
    //             });
    //         })->get();

    //     // Check if any events were found; if not, the room is available
    //     if ($events->isEmpty()) {
    //         return false; // Room is not taken
    //     }

    //     // Room is taken if there are overlapping events
    //     return true;
    // }
